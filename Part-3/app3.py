...
def fetch_data():
    try:
        ...
        groups_carts = pd.DataFrame(cursor.fetchall(), columns=['group_id', 'quantity'])
...
def calculate_average_quantities(groups_carts, single_deals):
    ...
    average_group_quantities = groups_carts.groupby('group_id')['quantity'].mean().reset_index()
    ...
    combined = pd.concat([average_group_quantities, average_single_quantities], ignore_index=True)
    return combined
...
if not groups_carts.empty and not single_deals.empty:
    ...
    average_quantities = calculate_average_quantities(groups_carts, single_deals)
